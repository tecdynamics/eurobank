...
class HookServiceProvider extends ServiceProvider
{
    ...
    {
        add_filter(PAYMENT_FILTER_ADDITIONAL_PAYMENT_METHODS, [$this, 'registerEurobankMethod'], 11, 2);
        add_filter(PAYMENT_FILTER_AFTER_POST_CHECKOUT, [$this, 'checkoutWithEurobank'], 11, 2);
    ...
    {
        $apiKey = get_payment_setting('key', EUROBANK_PAYMENT_METHOD_NAME);
        $apiSecret = get_payment_setting('secret', EUROBANK_PAYMENT_METHOD_NAME);
    ...
    {
        if ($request->input('payment_method') == EUROBANK_PAYMENT_METHOD_NAME) {
            try {
                $api = new Api(get_payment_setting('key', EUROBANK_PAYMENT_METHOD_NAME),
                    get_payment_setting('secret', EUROBANK_PAYMENT_METHOD_NAME));

                $api->utility->verifyPaymentSignature([
                    'EUROBANK_signature'  => $request->input('EUROBANK_signature'),
                    'EUROBANK_payment_id' => $request->input('EUROBANK_payment_id'),
                    'EUROBANK_order_id'   => $request->input('EUROBANK_order_id'),
                ]);

                $order = $api->order->fetch($request->input('EUROBANK_order_id'));
                ...
                if (in_array($order['status'], ['paid', 'attempted'])) {
                    ...
                    $data['charge_id'] = $request->input('EUROBANK_payment_id');
